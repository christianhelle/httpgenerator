 ...
 */
async function getInstalledHttpGeneratorVersion() {
 ...
 */
async function installHttpGenerator(forceReinstall = false) {
    ...
    const choice = await vscode.window.showInformationMessage('The httpgenerator .NET tool is not installed or the version does not match. Would you like to install/update it?', installOption, cancelOption);
 ...
 */
async function executeHttpGenerator(filePath, outputType) {
...
function activate(context) {
    ...
    let generateSingleHttpFileCommand = vscode.commands.registerCommand('http-file-generator.generateSingleHttpFile', async (fileUri) => {
        if (!fileUri) {
    ...
    let generateMultipleHttpFilesCommand = vscode.commands.registerCommand('http-file-generator.generateMultipleHttpFiles', async (fileUri) => {
        if (!fileUri) {
            // No file provided, prompt user to select one
            fileUri = await promptForOpenApiFile();
            if (!fileUri) {
    ...
    let generateSingleHttpFileMenuCommand = vscode.commands.registerCommand('http-file-generator.generateSingleHttpFileMenu', async (fileUri) => {
        // Context menu always has a file URI
        if (fileUri) {
            await executeHttpGenerator(fileUri.fsPath, 'OneFile');
        }
    });
    let generateMultipleHttpFilesMenuCommand = vscode.commands.registerCommand('http-file-generator.generateMultipleHttpFilesMenu', async (fileUri) => {
        // Context menu always has a file URI
        if (fileUri) {
            await executeHttpGenerator(fileUri.fsPath, 'OneRequestPerFile');
        }
