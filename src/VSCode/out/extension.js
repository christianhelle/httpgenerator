 ...
 */
async function executeProcessWithProgress(options) {
    return vscode.window.withProgress({
        ...
        return new Promise((resolve, reject) => {
            const process = (0, child_process_1.spawn)(options.command, options.args);
            let errorOutput = '';
 ...
 */
function isPathWithinWorkspace(inputPath) {
...
async function handleFileSelection() {
    ...
    const selectedFile = await vscode.window.showQuickPick(safeFiles.map(file => file.fsPath), { placeHolder: MESSAGES.FILE_PICKER_PLACEHOLDER });
    if (!selectedFile) {
        return undefined;
    }
    return selectedFile;
